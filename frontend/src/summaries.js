...
function GetButlerURL(repoName) {
  const repoUrls = (() => {
    try {
      return JSON.parse(import.meta.env.REPO_URLS);
    } catch (err) {
      console.error(`Could not parse REPO_URLS env var, ${err}`);
      console.error(`REPO_URLS='${import.meta.env.REPO_URLS}'`);
      return {};
    }
  })();
...
function GetClient() {
  const client = new S3Client({
    endpoint: import.meta.env.BUCKET_URL,
    region: "s3dfrgw",
    forcePathStyle: true,
    credentials: {
...
async function GetSummary(repoName, collectionName) {
  if (!import.meta.env.ENABLE_TEST_IMAGES) {
