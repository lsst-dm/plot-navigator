...
export function DropdownOptions({options, prefix, selected, onChange}) {

  const [expanded, setExpanded] = useState(false)
  /*
  const [selectedOption, setSelectedOption] = useState(defaultOption ? selectedOption : options[0])
  */
  ...
  return (
    ...
    {!expanded ? "" :
      <div ref={ref} className="border-2 border-gray-300 rounded-lg absolute z-1 bg-white my-1 drop-shadow-sm">
        {options.map((option) => (
          <div key={option} className={`px-2 py-1 cursor-pointer hover:bg-gray-200 ${option == selected ? "bg-buttons/40 ": ""}`}
            onClick={(e) => {
              setExpanded(false);
              onChange(option);
              }}>
