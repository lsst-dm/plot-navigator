...
export default function CompareCollectionButton({baseURL, collectionOptions}) {
    ...
    const officialOptions = collectionOptions.filter(option => !option.collection.startsWith("u/")).sort((a,b) => (b.lastModified - a.lastModified))
    const userOptions = collectionOptions.filter(option => option.collection.startsWith("u/")).sort((a,b) => (b.lastModified - a.lastModified))
                        ...
                        <select className="m-1 p-1" value={selectedOfficial} onChange={(e) => {
                            ...
                            { 
                                officialOptions.map(collectionOption =>
                                    <option key={makeOptionString(collectionOption)}
